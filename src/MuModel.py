...
class MuADSE():
    ...
    def get_model(self,summary=False):
        ...
        # 构建v,d的相关结构
        if self.task=="rate":
            model = self.__get_rating_model()
        elif self.task=="ctr":
            model = self.__get_ctr_model()
        
        else:
            model = self.__get_mlt_model()
        # model.load_weights("/mnt/Disk3/ysq/localFile/CKPT_DIR/rateHTI_Musical_Instruments_5/DoubleEmbeddingModel2/DoubleEmbeddingModel2.h5", by_name=True)
        # print("load success")
        # model.load_weights("/mnt/Disk3/ysq/localFile/CKPT_DIR/ctrHTI_Musical_Instruments_5/DoubleEmbeddingModel2/DoubleEmbeddingModel2.h5", by_name=True)
        print("load success")
        self.model = model
    ...
    def train_subfit_subdata(self,data_loader,validation_split=0.1):
        ...
        if self.task=="rate":
            ...
            self.premodel.fit(pre_train_data, label[:int(len(label)/ratio)], 
                                    epochs=15, verbose=1,batch_size=self.batch_size,
                                    callbacks=[cp_callback],
                                    validation_data=(valid_data,v_label),
                                    validation_freq=1)
            ...
            label[int(len(label)/ratio):] = pred_r[int(len(label)/ratio):,0]
            self.model.compile(
                optimizer="adam",
